...
get_datetime() {
  # Get format from publicbox.conf
...
if [ -z $1 ] || [ -z $2 ] ; then
  ...
  echo "Usage $0 <path to publicbox.conf> <step>"
...
if [ "$2" = "install" ] ; then
  ...
  echo "#--- Crontab for PublicBox-Timesave" >> $PUBLICBOX_FOLDER/tmp/crontab
  echo " */5 * * * * $PUBLICBOX_FOLDER/bin/timesave.sh $PUBLICBOX_FOLDER/conf/publicbox.conf save" >> $PUBLICBOX_FOLDER/tmp/crontab
  ...
  echo "  on OpenWrt run: /etc/init.d/publicbox enable"
