...
#    licenced with GPL-3
#
# Generate severall configuration files out of publicbox.conf
...
generate_lighttpd_env() {
	...
	LIGHTTPD_ENV="setenv.add-environment = ( 
	   \"PYTHONPATH\"             => \"$PYTHONPATH:$PUBLICBOX/python_lib\", 
...
if [ -z  $1 ] ; then
  echo "Usage is 
      generate_config_files.sh /opt/publicbox/conf/publicbox.conf
   "
   exit 255
fi
...
. $NODE_CONFIG
. $PUBLICBOX_FOLDER/lib/node_name_generation.sh

IPV6="#"

set_pathnames  $PUBLICBOX_FOLDER
...
generate_lighttpd_env $GLOBAL_CHAT "$GLOBAL_DEST" $PUBLICBOX_PYTHONPATH $GEN_CHATFILE $PUBLICBOX_FOLDER  $CHATFILE $SHOUTBOX_CLIENT_TIMESTAMP $UPLOADFOLDER $GEN_DISKFILE 
...
### PublicBox Scripts generates its own config in  /opt/publicbox/conf/avahi
###   but, the daemon works per default only on /etc/avahi
### If you want to enable avahi, then you have to link /etc/avahi to /opt/publicbox/conf/avahi
### On OpenWRT this should happen, if avahi is available before installing the publicbox
###  automtically. 
