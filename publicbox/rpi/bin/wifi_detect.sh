...
CONFIG_PATH="/opt/publicbox/conf/hostapd.conf"
PUBLICBOX_CONFIG_PATH="/opt/publicbox/conf/publicbox.conf"
...
publicbox_interface=$( grep -e '^INTERFACE' "${PUBLICBOX_CONFIG_PATH}" | \
                sed -e 's|INTERFACE=||' -e 's|"||g' )
dnsmasq_interface=$( grep -e '^DNSMASQ_INTERFACE' "${PUBLICBOX_CONFIG_PATH}" | \
                sed -e 's|DNSMASQ_INTERFACE=||' -e 's|"||g' )

sed -i -e "s|interface=$hostap_interface|interface=$WIFI_DEVICE|" "${CONFIG_PATH}"

#Only change publicbox interface if it is a wifi interface
if echo "$publicbox_interface" | grep -q "wlan" ; then
    sed -i -e "s|INTERFACE=\"$publicbox_interface\"|INTERFACE=\"$WIFI_DEVICE\"|" \
         "${PUBLICBOX_CONFIG_PATH}" 
fi
...
if [ -f /boot/wpa_supplicant.conf ]; then
  /opt/publicbox/rpi/bin/run_client.sh
  exit 1
  # Exit =! 0 will result in not starting publicbox service
fi
